...
class ImageDataset(Dataset):
    def __init__(self, root, transforms_=None, unaligned=False, count=-1, mode='train'):
        self.transform = transforms.Compose(transforms_)
        self.unaligned = unaligned
        self.count = count
        self.files_A = sorted(
            glob.glob(os.path.join(root, 'shadow_train') + '/*.*'))
        self.files_B = sorted(
            glob.glob(os.path.join(root, 'shadow_free') + '/*.*'))
